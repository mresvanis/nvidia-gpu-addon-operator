...
var _ = Describe("NicClusterPolicy Resource Reconcile", Ordered, func() {
	Context("Reconcile", func() {
		common.ProcessConfig()
		rrec := &NicClusterPolicyResourceReconciler{}
		...
		var cp netopv1alpha1.NicClusterPolicy

		Context("when GPUAddon RDMA is configured", func() {
			gpuAddon := &addonv1alpha1.GPUAddon{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test",
					Namespace: "test",
			...
			It("should create the NicClusterPolicy", func() {
				c := fake.
					NewClientBuilder().
					WithScheme(scheme).
					WithRuntimeObjects().
				...
				Expect(cond[0].Type).To(Equal(NicClusterPolicyDeployedCondition))
		...
		Context("when GPUAddon RDMA is not configured", func() {
			gpuAddon.Spec.RDMA = nil

			It("should not create the NicClusterPolicy", func() {
				c := fake.
					NewClientBuilder().
					WithScheme(scheme).
					WithRuntimeObjects().
	...
	Context("Delete", func() {
		...
		It("should delete the NicClusterPolicy", func() {
			c := fake.
				NewClientBuilder().
				WithScheme(scheme).
